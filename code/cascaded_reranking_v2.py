"""
Cascaded Reranking v2: 4-Stage LLM Reranking (더 세밀한 필터링)
- Stage 0: Top 50 초기 검색
- Stage 1: Top 50 → Top 30 (1차 필터링)
- Stage 2: Top 30 → Top 20 (2차 필터링)
- Stage 3: Top 20 → Top 10 (3차 필터링)
- Stage 4: Top 10 → Top 3 (최종 정밀 Reranking)

목표: Cascaded 단계 증가로 3점 문서의 정보 손실 최소화
가설: v1의 30→10 급격한 필터링이 3점 문서를 누락시킴
...
def search_bgem3_hybrid(query, embeddings_dict, top_k=30, max_length=128):
    """
    BGE-M3 Hybrid 검색 (Top 50 지원)
    """
    # BGE-M3 쿼리 임베딩
...
def hybrid_search_rrf(query, embeddings_dict, top_k=50, k=60, query_max_length=128):
    """
    RRF로 BM25 + BGE-M3 Hybrid 결합
    top_k를 50으로 증가 (Cascaded Reranking v2용)
    """
    # BM25 검색
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": f"당신은 과학 지식 검색 시스템의 {stage_name} 필터링 전문가입니다."
                },
                {
                    "role": "user",
                    "content": f"""쿼리: {query}
...
def llm_final_rerank(query, docs, top_k=3):
    """
    최종 Stage: Top 10 → Top 3 정밀 Reranking
    """
    if not docs or len(docs) <= top_k or not client:
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "당신은 과학 지식 검색 시스템의 최종 relevance 판단 전문가입니다. 주어진 쿼리에 가장 관련성 높은 문서를 정확히 선택하세요."
                },
                {
                    "role": "user",
                    "content": f"""쿼리: {query}
...
def llm_cascaded_rerank_v2(query, docs, top_k=3):
    """
    Cascaded (4-Stage) Reranking v2

    Stage 1: Top 50 → Top 30 (1차 필터링)
    Stage 2: Top 30 → Top 20 (2차 필터링)
    Stage 3: Top 20 → Top 10 (3차 필터링)
    Stage 4: Top 10 → Top 3 (최종 정밀 판단)

    목표: 더 세밀한 필터링으로 3점 문서 보존
    """
...
def cascaded_reranking_strategy(eval_id, msg, embeddings_dict):
    """
    Cascaded Reranking v2 전략

    1. LLM 쿼리 재작성 (멀티턴 대화 맥락 통합)
    2. Hybrid Search (Top 50으로 확장)
    3. Cascaded LLM Reranking (50 → 30 → 20 → 10 → 3)
    """
    # 일반 대화는 빈 결과
    ...
    # Step 2: Hybrid Search (Top 50으로 확장)
    ...
    # Step 3: Cascaded LLM Reranking (50 → 30 → 20 → 10 → 3)
    final_topk = llm_cascaded_rerank_v2(rewritten_query, hybrid_results, top_k=3)

    return final_topk

def run_experiment():
    """Cascaded Reranking v2 실험 실행"""
    print("="*80)
    print("Cascaded Reranking v2 실험")
    print("="*80)
    print("Stage 1: Top 50 → Top 30")
    print("Stage 2: Top 30 → Top 20")
    print("Stage 3: Top 20 → Top 10")
    print("Stage 4: Top 10 → Top 3")
    print("="*80)
    ...
    for item in tqdm(eval_data, desc="Cascaded Reranking v2"):
        eval_id = item['eval_id']
        msg = item['msg']

        # Cascaded Reranking v2 전략 실행
    ...
    # 제출 파일 생성
    output_path = 'cascaded_reranking_v2_submission.csv'
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("Cascaded Reranking v2: 4-Stage LLM Reranking")
    print("목표: 더 세밀한 필터링으로 3점 문서 보존")
    print("예상 성능: 0.78~0.80 MAP@3")
    print("="*80)
    run_experiment()
