"""
LLM 최적화 솔루션 - BM25 신뢰 기반

핵심 변경:
1. BM25 Top-3를 기본 신뢰 (관련도 있다고 가정)
2. LLM은 순위만 조정 (제외하지 않음)
3. Query Rewriting으로 검색 품질 향상
...
def main():
    ...
    process_with_llm_optimized(
        ...
        output_path='llm_optimized_submission.csv'
    )
    ...
    print(f"{'='*80}")
    print(f"\n생성된 파일:")
    print(f"  - llm_optimized_submission.csv")
    print(f"\n핵심 변경사항:")
    print(f"  1. BM25 Top-3를 신뢰 (제외하지 않음)")
    print(f"  2. LLM은 순위만 조정")
    print(f"  3. Query Rewriting으로 검색 개선")
    print(f"  4. TopK=3 고정 (super_simple 전략)")
    print(f"\n기대 효과:")
    print(f"  - Query Rewriting: 검색 품질 향상")
    print(f"  - LLM Reranking: 순위 정확도 향상")
    print(f"  - BM25 신뢰: Recall 유지")
    print(f"  - 예상 MAP: 0.70~0.80")
    print(f"\n왜 더 나은가:")
    print(f"  - 이전 실패: LLM이 너무 보수적 (TopK=0 74%)")
    print(f"  - 현재: BM25 Top-3를 모두 사용")
    print(f"  - LLM의 장점만 활용 (순위 개선)")
    print(f"{'='*80}\n")
