"""
Overlap Chunking v1 - Phase 2 테스트

핵심 변경:
1. Semantic Chunking + 1문장 Overlap으로 문맥 연속성 보장
2. original_docid 기반 중복 제거
3. BM25 Top-3 신뢰 + LLM Reranking

목표: 0.6864 → 0.688-0.693 (+0.4-0.9%)
"""
...
def main():
    print("=" * 80)
    print("Overlap Chunking v1 - Phase 2 테스트")
    print("=" * 80)
    ...
    process_with_llm_optimized(
        ...
        output_path='solar_semantic_overlap_v1_submission.csv'
    )

    print(f"\n{'='*80}")
    print(f"✅ Overlap Chunking v1 완료")
    print(f"{'='*80}")
    print(f"\n생성된 파일:")
    print(f"  - solar_semantic_overlap_v1_submission.csv")
    print(f"\n핵심 변경사항:")
    print(f"  1. Semantic Chunking + 1문장 Overlap으로 문맥 연속성 보장")
    print(f"  2. original_docid 기반 중복 제거")
    print(f"  3. BM25 Top-5 → 중복 제거 → Top-3")
    print(f"  4. LLM Reranking으로 순위 정밀화")
    print(f"\n기대 효과:")
    print(f"  - Overlap Chunking: 문맥 연속성 향상")
    print(f"  - 중복 제거: 다양성 증가")
    print(f"  - 예상 MAP: 0.688-0.693 (+0.4-0.9%)")
    print(f"\nElasticsearch 통계:")
    print(f"  - 원본 문서: 4,272개")
    print(f"  - 청크 문서: 6,186개 (vs Phase1: 5,613개)")
    print(f"  - 청킹 비율: 13.8% (589개 문서)")
    print(f"  - Overlap: 1문장 중복")
    print(f"{'='*80}\n")
