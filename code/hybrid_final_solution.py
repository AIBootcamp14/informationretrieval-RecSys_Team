"""
최종 Hybrid Search 솔루션
BM25 + Dense Retrieval + Solar LLM Reranking
목표: MAP@3 0.9+ 달성
"""
...
def search_dense(query, embeddings_dict, top_k=10):
    """Dense Retrieval 검색"""
    # 쿼리 임베딩
...
def hybrid_search_rrf(query, embeddings_dict, top_k=20, k=60):
    """
    Reciprocal Rank Fusion (RRF)로 BM25 + Dense 결합

    Args:
        query: 검색 쿼리
        embeddings_dict: Dense 임베딩 딕셔너리
...
def hybrid_final_strategy(eval_id, query, embeddings_dict):
    """최종 Hybrid 전략"""
    # 일반 대화는 빈 결과
    ...
    # Hybrid Search (RRF)
    hybrid_results = hybrid_search_rrf(query, embeddings_dict, top_k=20)
...
def run_final_experiment():
    """최종 실험 실행"""
    print("="*80)
    print("최종 Hybrid Search 실험")
    print("="*80)
    print("전략: BM25 + Dense (RRF) + Solar LLM Reranking")
    print("="*80)
    ...
    for item in tqdm(eval_data, desc="Hybrid Search"):
        ...
        # Hybrid 전략 실행
        topk = hybrid_final_strategy(eval_id, query, embeddings_dict)
    ...
    # 제출 파일 생성
    output_path = 'hybrid_final_submission.csv'
