...
def reindex_with_semantic_chunking(documents_path='../data/documents.jsonl'):
    """Aggressive Chunking 기반 재인덱싱"""
    print(f"\n{'='*80}")
    print("Aggressive Chunking 재인덱싱")
    print(f"{'='*80}")
    ...
    # 재인덱싱
    print(f"\n🔄 재인덱싱 중...")
    for doc in tqdm(documents, desc="Aggressive Chunking"):
        docid = doc['docid']
        content = doc['content']
        total_original_docs += 1

        # Aggressive Chunking (작은 청크)
        chunks = semantic_sentence_chunking(
            content,
            max_chunk_size=250,
            min_chunk_size=80,
            similarity_threshold=0.70,  # 높은 임계값으로 더 자주 분할
            overlap_sentences=0
        )
...
def main():
    print("="*80)
    print("Elasticsearch Aggressive Chunking 재인덱싱")
    print("="*80)
    ...
    print(f"\n💡 다음 단계:")
    print(f"  1. solar_semantic_aggressive_v1.py 실행으로 새 인덱스 테스트")
    print(f"  2. 결과가 좋으면 유지, 나쁘면 롤백:")
    print(f"     롤백 명령: python rollback_index.py")
    print(f"\n📊 Aggressive Chunking 설정:")
    print(f"  - max_chunk_size: 250자 (vs Phase1: 400자)")
    print(f"  - min_chunk_size: 80자 (vs Phase1: 100자)")
    print(f"  - similarity_threshold: 0.70 (vs Phase1: 0.65)")
    print(f"  - 목표: 더 많은 청크 생성으로 정밀 검색")
