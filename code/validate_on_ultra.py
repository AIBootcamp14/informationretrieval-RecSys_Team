...
- eval_id 205: 피를 맑게 하고 노폐물을 없애는 기관 (신장)
- eval_id 43: 달의 같은 면만 보이는 이유 (조석 고정)
- eval_id 47: 원자번호와 소립자의 관계 (양성자)
"""
...
def load_ultra_validation(path='ultra_validation_solar.jsonl'):
    ...
    print(f"✓ Loaded {len(validation_data)} ultra-high-quality validation samples")
...
def evaluate_cascaded_reranking(validation_data, embeddings_dict):
    ...
    for val_item in validation_data:
        ...
        print(f"  → AP@3: {ap:.4f} | Hits: {results[-1]['hits']}/3")
...
def print_detailed_results(results):
    ...
    for i, result in enumerate(results, 1):
        ...
        for j, docid in enumerate(result['predicted'], 1):
            hit = "✓" if docid in result['ground_truth'] else "✗"
            print(f"  {j}. {docid} {hit}")
    ...
    if map_score > 0.7848:
        ...
        print(f"✅ IMPROVEMENT: +{improvement:.4f} ({improvement/0.7848*100:.1f}%)")
        print(f"{'='*80}")
        print(f"🎯 RECOMMENDATION: Cascaded reranking performs BETTER than baseline!")
        print(f"   → Submit to competition: cascaded_reranking_v1_submission.csv")
        print(f"   → Expand validation set to confirm improvement")
    elif abs(map_score - 0.7848) < 0.05:  # Within 5% (0.05 absolute difference)
        diff = abs(map_score - 0.7848)
        print(f"≈  SIMILAR: ±{diff:.4f} ({diff/0.7848*100:.1f}%)")
        print(f"{'='*80}")
        print(f"🤔 RECOMMENDATION: Performance is SIMILAR to baseline")
        print(f"   → Validation approach seems reliable")
        print(f"   → Expand validation set (lower Phase 2 threshold to 3)")
        print(f"   → Generate 30-50 samples for better statistical confidence")
    else:
        decline = 0.7848 - map_score
        print(f"❌ DECLINE: -{decline:.4f} ({decline/0.7848*100:.1f}%)")
        print(f"{'='*80}")
        print(f"⚠️  RECOMMENDATION: Performance is WORSE than baseline")
        print(f"   → Review Solar Pro's labeling criteria")
        print(f"   → Investigate discrepancy between validation and real performance")
        print(f"   → May need to adjust validation pipeline")
...
def main():
    ...
    print("\n📂 Loading ultra validation samples...")
