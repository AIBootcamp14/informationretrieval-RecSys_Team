"""
Query Decomposition v1: Multi-Query Search with RRF Merge

전략:
1. LLM으로 복잡한 쿼리를 3-5개 서브 쿼리로 분해
2. 각 서브 쿼리로 Hybrid Search 수행
3. RRF로 모든 결과 통합 (Top 30)
4. Cascaded Reranking (30 → 10 → 3)

목표: Retrieval Recall 향상 (Task 3에서 발견한 85.7% retrieval 실패 해결)
예상 성능: 0.79 → 0.82+ MAP@3
"""
...
def decompose_query(query):
    """
    Query Decomposition: 복잡한 쿼리를 3-5개 서브 쿼리로 분해
    ...
    # 단순 쿼리는 분해하지 않음 (5단어 이하)
    if len(query.split()) <= 5 and '?' not in query[-3:]:
        return [query]

    try:
        prompt = f"""다음 과학 질문을 3-5개의 더 구체적인 서브 질문으로 분해하세요:

질문: {query}

서브 질문은 다음 원칙을 따르세요:
1. 원래 질문의 다양한 측면을 다뤄야 합니다
2. 각 서브 질문은 독립적으로 검색 가능해야 합니다
3. 너무 좁거나 너무 넓지 않게 만드세요
4. 원래 질문의 핵심 개념을 모두 포함해야 합니다
...
설명 없이 서브 질문만 출력하세요."""
        ...
        for line in result.split('\n'):
            ...
            elif line and not line.startswith('#') and not line.startswith('출력'):
        ...
        print(f"Query decomposition 실패: {e}")
        return [query]
...
def multi_query_search_rrf(queries, embeddings_dict, top_k=30, k=60):
    """
    Multi-Query Search with RRF Merge

    여러 개의 쿼리로 각각 검색한 뒤 RRF로 통합
    """
    all_rrf_scores = {}
    all_doc_contents = {}
    ...
    for query in queries:
        # Hybrid Search (BM25 + BGE-M3)
        hybrid_results = hybrid_search_rrf(query, embeddings_dict, top_k=20, k=k)

        # RRF 스코어 누적
        for rank, doc in enumerate(hybrid_results, 1):
            docid = doc['docid']
            all_rrf_scores[docid] = all_rrf_scores.get(docid, 0) + 1 / (k + rank)
            all_doc_contents[docid] = doc['content']

    # 정렬
    sorted_docs = sorted(all_rrf_scores.items(), key=lambda x: x[1], reverse=True)

    results = []
    for docid, score in sorted_docs[:top_k]:
        results.append({
            'docid': docid,
            'content': all_doc_contents[docid],
            'score': score
        })
...
def query_decomposition_strategy(eval_id, msg, embeddings_dict):
    """
    Query Decomposition v1 전략

    1. LLM 쿼리 재작성 (멀티턴 대화 맥락 통합)
    2. Query Decomposition (3-5개 서브 쿼리)
    3. Multi-Query Search with RRF (Top 30)
    4. Cascaded LLM Reranking (30 → 10 → 3)
    """
    # 일반 대화는 빈 결과
    ...
    print(f"\n[eval_id={eval_id}]")
    print(f"원본 쿼리: {rewritten_query}")
    print(f"서브 쿼리 ({len(sub_queries)}개):")
    for i, sq in enumerate(sub_queries, 1):
        print(f"  {i}. {sq}")

    # Step 3: Multi-Query Search with RRF Merge (Top 30)
    merged_results = multi_query_search_rrf(
        sub_queries,
        embeddings_dict,
        top_k=30,
        k=60
    ...
    # Step 4: Cascaded LLM Reranking (30 → 10 → 3)
    final_topk = llm_cascaded_rerank(rewritten_query, merged_results, top_k=3)

    return final_topk

def run_experiment():
    """Query Decomposition v1 실험 실행"""
    print("="*80)
    print("Query Decomposition v1 실험")
    print("="*80)
    print("Step 1: Query Decomposition (3-5개 서브 쿼리)")
    print("Step 2: Multi-Query Search with RRF Merge")
    print("Step 3: Cascaded Reranking (30 → 10 → 3)")
    print("="*80)
    ...
    for item in tqdm(eval_data, desc="Query Decomposition v1"):
        eval_id = item['eval_id']
        msg = item['msg']

        # Query Decomposition 전략 실행
        topk = query_decomposition_strategy(eval_id, msg, embeddings_dict)
    ...
    # 제출 파일 생성
    output_path = 'query_decomposition_v1_submission.csv'
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("Query Decomposition v1: Multi-Query Search with RRF")
    print("목표: Retrieval Recall 향상 (85.7% retrieval 실패 해결)")
    print("예상 성능: 0.79 → 0.82+ MAP@3")
    print("="*80)
    run_experiment()
