"""
LLM 최적화 솔루션 - OpenAI API 버전

Solar API 대신 OpenAI API 사용:
- Query Rewriting: gpt-4o (최신 모델)
- Reranking: gpt-4o (최신 모델)

목표: Solar vs OpenAI 성능 비교
"""
...
def main():
    ...
    print("✅ OpenAI API Key 확인")

    # 처리 실행
    process_with_llm_optimized(
        eval_path='../data/eval.jsonl',
        output_path='llm_optimized_openai_submission.csv'
    )
    ...
    print(f"{'='*80}")
    print(f"\n생성된 파일:")
    print(f"  - llm_optimized_openai_submission.csv")
    print(f"\n사용 모델:")
    print(f"  - Query Rewriting: gpt-4o (최신)")
    print(f"  - Reranking: gpt-4o (최신)")
    print(f"\n비교 대상:")
    print(f"  - llm_optimized_submission.csv (Solar API)")
    print(f"\n예상 효과:")
    print(f"  - OpenAI GPT-4o의 뛰어난 언어 이해력")
    print(f"  - Solar와 성능 비교 가능")
    print(f"  - 예상 MAP: 0.75~0.85")
    print(f"{'='*80}\n")
